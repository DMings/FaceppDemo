...
public class PointsMatrix {
    ...
    private final String fragmentShaderCode = "precision mediump float;" + "uniform vec4 vColor;" + "uniform bool flag;" + "void main() {"
            + "if (length(gl_PointCoord - vec2(0.5)) > 0.5 && flag) {" +
            "        discard;" +
            "    }" + "  gl_FragColor = vColor;" + "}";
...
//	float color[] = { 0.2f, 0.709803922f, 0.898039216f, 1.0f };
    float color[] = {0.49f, 0.73f, 1f, 1.0f};
...
//			{ 0.0f, 0.0f, 1f, 1.0f } }; // blue
    float color_megvii[][] = {
            {0f, 0f, 0f, 1.0f},
            {1.0f, 1.0f, 1.0f, 1.0f},
            {0.38f, 0.36f, 0.9f, 1.0f}, //red
            {0.36f, 0.95f, 1f, 1.0f}, //green
            {0.84f, 0.0f, 0.52f, 1.0f}}; // blue
    ...
    // 画底部矩形
    public ArrayList<FloatBuffer> bottomVertexBuffers = new ArrayList<FloatBuffer>();
    // 画底部矩形
    ...
    public void draw(float[] mvpMatrix) {
        ...
        mFlagHandle = GLES20.glGetUniformLocation(mProgram, "flag");
        // get handle to shape_debug_bg's transformation matrix
        ...
        GLES20.glUniform1i(mFlagHandle, 1);
